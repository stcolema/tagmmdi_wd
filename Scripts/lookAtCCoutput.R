...
fused_gene_probs_13 <- colSums(cc_out$allocations[ , , 1] == cc_out$allocations[ , , 3] ) / D
fused_genes_13 <- which(fused_gene_probs_13 > 0.5)

fused_gene_probs_23 <- colSums(cc_out$allocations[ , , 2] == cc_out$allocations[ , , 3] ) / D
...
microarray_data |> 
  mutate(Predicted_label = factor(cc_out$predicted_partitions[[1]]),
