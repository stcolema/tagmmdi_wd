...
#! Example call:
# sbatch callTagmMDICVSingleFold.sh -d "'HEK293T2011 HEK293T2011goCC'" -r 10000 -b 2000 -t 50 -s 1 -i 3;
...
save_dir='/home/sdc56/rds/hpc-work/tagmmdi/CV_output/test_';
K=100;
...
# Ensure all classes are represented in the training data
adjust_for_TL=false;

# Receive parameters from command line
while getopts r:t:b:d:k:c:n:i:a: option; do
    case "${option}" in
        r) R=${OPTARG};;
        t) thin=${OPTARG};;
        b) burn=${OPTARG};;
...
# Capitalise the adjustment argument
...
# Load an R module and a compiler
module load gcc/11
module load R/4.1.0-icelake

# Used by RcppParallel
module load tbb-2017.4-gcc-5.4.0-t5fcc23

# used by some Bioconductor packages
module load netcdf/4.4.1


#! Full path to application executable: 
application="Rscript ${script_dir}mdiTagmCVSingleLoop.R --datasets ${datasets} --R ${R} --thin ${thin} --burn ${burn} --seed ${seed} --test_size ${test_size} --K ${K} --n_chains ${n_chains} --save_dir ${save_dir} --categorical_column_threshold ${cat_threshold} --adjust_for_TL ${adjust_for_TL}"
