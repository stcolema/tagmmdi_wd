...
while getopts r:t:d:k:i:s:z: option; do
    case "${option}" in
        r) R=${OPTARG};;
        t) thin=${OPTARG};;
        d) data_file=${OPTARG};;
...
application="Rscript ${script_dir}${script} --data_file ${data_file} --R ${R} --thin ${thin} --seed ${seed} --K ${K} --n_chains ${n_chains} --save_dir ${save_dir} --type ${types}";
