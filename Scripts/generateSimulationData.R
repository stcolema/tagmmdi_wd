...
generateMDIDataLabels <- function(N, V, K, gammas, phis, n_iter = 10) {
  ...
  colnames(labels) <- paste0("View.", seq(1, V))
  labels
}
...
gammas <- matrix(c(
  4.0, 7.0, 7.0, 5.5, 4.0, 0.0, 0.0,
  4.0, 4.0, 4.0, 4.0, 4.0, 4.0, 0.0,
  6.0, 6.0, 3.0, 3.0, 4.0, 6.0, 4.0
), ncol = V)
...
for (n in seq(1, n_datasets)) {
  ...
  labels <- generateMDIDataLabels(N, V, K, gammas, phis, 3)
  ...
  fixed <-rep(0, N) # sample(c(0, 1), N, replace = TRUE, prob = c(1 - frac_known, frac_known))

  # strata needs size to be ordered as they appear in data
  label_representation <- table(labels[, 1])
  .size <- ceiling(label_representation * (1.0 - frac_known))
  .size <- .size[unique(labels[, 1])]
  
  if(any(.size == 0)) {
    classes_missing_training_samples <- which(.size == 0)
    .size[classes_missing_training_samples] <- 1
  }
  
  # get strata indices
  test.idx <- sampling::strata(labels, "View.1",
    size = .size,
    method = "srswor"
  )$ID_unit
  ...
  for (scn in scenarios) {
    if (scn %in% c("Gaussian", "MVT")) {
      ...
      group_means <- c(-1.25, 1.25, -0.50, 0.50, 1.25) #  rnorm(K_1, sd = 1.75)
      group_sds <- c(1.50, 1.25, 1.75, 2.00, 1.50) # rgamma(K_1, 4, 2)
      group_dfs <- c(5, 5, 5, 5, 5) # , 5, 10)

      params_2 <- list(
        means = c(-1.00, -0.50, -0.75, 0.00, 0.25, 0.50, 0.50, 0.00), # rnorm(K_2, sd = 0.85),
        std_devs = c(0.5, 1.0, 0.5, 1.0, 0.5, 0.25, 1.25, 0.5, 1.0), # rgamma(K_2, 2, 2),
        dfs = c(5, 5, 5, 5, 5, 5)
      )
      params_3 <- list(
        means = c(-0.25, -0.50, -1.25, -0.50, 0.50, 1.00, 0.00), # rnorm(K_3, sd = 1.05),
        std_devs = c(0.50, 0.75, 1.50, 1.00, 0.50, 0.75, 1.25), # rgamma(K_3, 2, 2),
        dfs = c(5, 5, 5, 5, 5, 5, 5)
      )
    }

    if (scn == "LogPoisson") {
      rates <- c(5, 15, 25, 35, 50, 5, 25) # rgamma(K_1, 2.5, 0.1) + 1
      params_2 <- list(rates = c(5, 15, 30, 30, 60, 45)) #  rgamma(K_2, 2.5, 0.1) + 1)
      params_3 <- list(rates = c(5, 8, 13, 21, 34, 55, 21)) #, 60, 55, 28)) # rgamma(K_3, 2.5, 0.1) + 1)
    }
