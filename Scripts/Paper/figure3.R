...
plot_df <- data.frame("tSNE_1" = my_tsne$Y[, 1], 
                      "tSNE_2" = my_tsne$Y[, 2], 
                      markers = markers, 
...
secretory_gene_expression_data <- microarray_mat[secretory_proteins, ]
...
ggsave("~/Desktop/fig3GE.png", plot = p_ge, height = 6.0, width = 8.0)
ggsave("~/Desktop/fig3tSNE.png", plot = p_tsne, height = 6.0, width = 16.0)
...
hull_data <- plot_df |>
  filter(predicted_localisation %in% secretory_organelles) |> 
  group_by(predicted_localisation) |> 
  summarise(tSNE_1 = mean(tSNE_1), tSNE_2 = mean(tSNE_2))
  # slice(chull(tSNE_1, tSNE_2))

plot_df |>
  filter(tSNE_1 > 17, tSNE_1 < 19, tSNE_2 < -22, tSNE_2 > -25) |> 
  filter(organelle %in% secretory_organelles) |> 
...
plot_df |>
  # filter(tSNE_1 > 0, tSNE_1 < 55, tSNE_2 < 27.5, tSNE_2 > -25) |> 
  # filter(organelle %in% secretory_organelles) |> 
  ggplot(aes(x = tSNE_1, y = tSNE_2, color = predicted_localisation)) +
  geom_point(alpha = 0.4) +
  labs(x = "tSNE 1", y = "tSNE 2", color = "Markers") +
  ...
  scale_color_manual(values = col_pal) +
  guides(color = guide_legend(ncol = 4)) +
  geom_circle(data = hull_data) 
  # geom_polygon(data = hull_data, alpha = 0.3)
                    
annotatedHeatmap(secretory_gene_expression_data, secretory_predictions, include_rownames = FALSE, cluster_cols = FALSE)
...
rel_protein_biochemical_properties <- protein_biochemical_properties[secretory_proteins, ]
secretory_tSNE_df <- plot_df[secretory_proteins, ]

secretory_df <- cbind(secretory_tSNE_df, rel_protein_biochemical_properties[, -c(2, 4, 5)]) 
# |>
  # pivot_longer(c(mdi.mcmc.allocation, tagm.mcmc.allocation), names_to = "Model", values_to = "Allocation")
