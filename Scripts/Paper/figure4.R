...
ggsave("~/Desktop/fig4GE.png", plot = p_ge, height = 6.0, width = 8.0)
ggsave("~/Desktop/fig4tSNE.png", plot = p_tsne, height = 6.0, width = 16.0)
...
dg_gene_expression_data[row_order, ][1:15, ] |> pheatmap::pheatmap(cluster_cols = FALSE, cluster_rows = FALSE)
dg_gene_expression_data[row_order, ][110:134, ] |> pheatmap::pheatmap(cluster_cols = FALSE, cluster_rows = FALSE)
dg_gene_expression_data[row_order, ][71:78, ] |> pheatmap::pheatmap(cluster_cols = FALSE, cluster_rows = FALSE)
dg_gene_expression_data[row_order, ][c(16:70, 79:109), ] |> pheatmap::pheatmap(cluster_cols = FALSE, cluster_rows = FALSE)
...
dg_df <- cbind(dg_tSNE_df, rel_protein_biochemical_properties[, -c(2, 4, 5)])
...
dg_df$pI <- as.numeric(dg_df$pI)
dg_df$Fixed <- dg_df$markers != "all other proteins"
p_pI <- dg_df |>
  ggplot(aes(x = GE_cluster, y = log(pI), fill = GE_cluster)) +
  # facet_wrap(~ Fixed) +
  geom_boxplot() +
  scale_fill_manual(values = ge_col_pal)

p_dNdS <- dg_df |>
  ggplot(aes(x = GE_cluster, y = log(dNdS), fill = GE_cluster)) +
  geom_boxplot() +
  scale_fill_manual(values = ge_col_pal)
