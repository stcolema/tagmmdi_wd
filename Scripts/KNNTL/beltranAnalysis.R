...
# required.
save_dir <- paste0(args$save_dir, "Beltran/")
...
beltran_24_markers <- pRoloc:::subsetAsDataFrame(beltran2016HCMV24, "markers", train = TRUE)
beltran_48_markers <- pRoloc:::subsetAsDataFrame(beltran2016HCMV48, "markers", train = TRUE)
beltran_72_markers <- pRoloc:::subsetAsDataFrame(beltran2016HCMV72, "markers", train = TRUE)
beltran_96_markers <- pRoloc:::subsetAsDataFrame(beltran2016HCMV96, "markers", train = TRUE)
beltran_120_markers <- pRoloc:::subsetAsDataFrame(beltran2016HCMV120, "markers", train = TRUE)

# Find the proteins in each dataset
b24_proteins <- row.names(beltran_24_markers)
b48_proteins <- row.names(beltran_48_markers)
b72_proteins <- row.names(beltran_72_markers)
b96_proteins <- row.names(beltran_96_markers)
b120_proteins <- row.names(beltran_120_markers)
...
# The expression data
beltran_24 <- beltran_24_markers[proteins_captured, -7]
beltran_48 <- beltran_48_markers[proteins_captured, -7]
beltran_72 <- beltran_72_markers[proteins_captured, -7]
beltran_96 <- beltran_96_markers[proteins_captured, -7]
beltran_120 <- beltran_120_markers[proteins_captured, -7]
...
true_allocations <- matrix(c(
  beltran_24_markers[proteins_captured, 7],
  beltran_48_markers[proteins_captured, 7],
  beltran_72_markers[proteins_captured, 7],
  beltran_96_markers[proteins_captured, 7],
  beltran_120_markers[proteins_captured, 7]
), N, L)
...
marker.data <- pRoloc::markerMSnSet(beltran2016HCMV96)
X <- beltran_24_markers[proteins_captured, ]
...
K <- c(
  length(pRoloc::getMarkerClasses(beltran2016HCMV24)),
  length(pRoloc::getMarkerClasses(beltran2016HCMV48)),
  length(pRoloc::getMarkerClasses(beltran2016HCMV72)),
  length(pRoloc::getMarkerClasses(beltran2016HCMV96)),
  length(pRoloc::getMarkerClasses(beltran2016HCMV120))
)


# Create a data frame of the classes present and their associated number
classes_pres <- pRoloc::getMarkerClasses(beltran2016HCMV24)
...
test_markers <- beltran_24_markers[proteins_captured, 7][test_inds]
...
X <- list(
  as.matrix(beltran_24),
  as.matrix(beltran_48),
  as.matrix(beltran_72),
  as.matrix(beltran_96),
  as.matrix(beltran_120)
)

# Do a fold of the cross validation for MDI and the single-view mixture model
types <- c("TAGM", "TAGM", "TAGM", "TAGM", "TAGM")
