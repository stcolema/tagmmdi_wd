...
mdi_samples_dir <- paste0(save_dir, "MDI_samples/")
tagm_samples_dir <- paste0(save_dir, "TAGM_samples/")

dir.create(mdi_samples_dir, showWarnings = FALSE)
dir.create(tagm_samples_dir, showWarnings = FALSE)
...
mdi_chains <- mdir::runMCMCChainsSavingToFile(X, n_chains, R, thin,
  types = types,
  K = K,
  fixed = fixed,
  initial_labels = initial_labels,
  dir_path = paste0(save_dir, "MDI_samples/")
)

mdi_chains$Seed <- seed

saveRDS(mdi_chains, file = mdi_save_name)
...
for (ll in seq(1, L)) {
  ...
  #   classes_pres,
  #   MDI_run = FALSE,
  #   dir_path = paste0(save_dir, "TAGM_samples/")
  # )
...
saveRDS(tagm_chains, file = tagm_save_name)
