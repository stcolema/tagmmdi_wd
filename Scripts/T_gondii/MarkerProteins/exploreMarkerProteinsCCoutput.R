...
inputdata_dir <- "./T_gondii/PreparedData/" #
save_dir <- "./T_gondii/MarkersOnlyOutput/CC/" #
model_output_dir <- "./T_gondii/MarkersOnlyOutput/CC/" #
...
microarray_file <- paste0(inputdata_dir, "markerProteinsCellCycleNormalised.csv") # paste0(data_dir, "ToxoDB_TgME49_Protein-coding_DNA_microarray.txt")
rna_seq_file <- paste0(inputdata_dir, "markerProteinsRNASeqMacrophage.csv") # paste0(data_dir, "ToxoDB_TgME49_Protein-coding_RNA-Seq.txt")
lopit_file <- paste0(inputdata_dir, "markerProteinsLOPIT.csv")
...
data_modelled <- readRDS(paste0(inputdata_dir, "TGondiiMDI_K_125_input_marker_proteins.rds"))
...
prediction_mat[fused_genes_23, 3]
...
tagm_comparison[which(tagm_comparison$tagm.mcmc.allocation != tagm_comparison$mdi.mcmc.allocation), ]
...
microarray_data |>
  mutate(
    Predicted_label = factor(cc_out$predicted_partitions[[1]]),
...
rna_seq_data |>
  mutate(
    Predicted_label = factor(cc_out$predicted_partitions[[2]]),
...
anno_row <- data.frame(
  Cluster = factor(paste("Cluster", cc_out$predicted_partitions[[1]][marker_genes])),
